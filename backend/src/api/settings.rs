...
pub async fn get_settings(
    ...
    let show_hidden_default =
        if let Some(val) = state.get_system_setting("show_hidden_default").await {
            val == "true"
        } else {
    ...
    let retry_attempts = state
        .get_system_setting("retry_attempts")
        .await
        .and_then(|s| s.parse::<usize>().ok())
        .unwrap_or(3);
    let conn_timeout = state
        .get_system_setting("connection_timeout_secs")
        .await
        .and_then(|s| s.parse::<u64>().ok())
        .unwrap_or(60);
    let log_level = state
        .get_system_setting("log_level")
        .await
...
pub async fn update_settings(
    ...
    if let Some(s) = payload.settings {
        // General
        state
            .set_system_setting("theme", &s.general.theme)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save theme: {}", e))?;
        state
            .set_system_setting("default_view", &s.general.default_view)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save default_view: {}", e))?;
        state
            .set_system_setting(
                "show_hidden_default",
                if s.general.show_hidden_default {
                    "true"
                } else {
                    "false"
                },
        ...
        // File Manager
        state
            .set_system_setting("default_layout", &s.file_manager.default_layout)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save default_layout: {}", e))?;
        state
            .set_system_setting(
                "max_editable_size",
                &s.file_manager.max_editable_size.to_string(),
            )
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save max_editable_size: {}", e))?;

        // Transfers (P1 #16 & #17: Dynamic update on TransferManager without restart)
        state
            .set_system_setting(
                "max_concurrent_transfers",
                &s.transfers.max_concurrent_transfers.to_string(),
            )
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save max_concurrent_transfers: {}", e))?;
        state
            .set_system_setting("retry_attempts", &s.transfers.retry_attempts.to_string())
            .await
            .map_err(|e| anyhow::anyhow!("Failed to save retry_attempts: {}", e))?;
        state.transfer_manager.update_limits(
            s.transfers.max_concurrent_transfers,
            s.transfers.retry_attempts,
        );
        ...
        state
            .set_system_setting(
                ...
                &s.connections.connection_timeout_secs.to_string(),
            )
            .await
        ...
        if !trimmed_root.is_empty() {
            ...
            state
                .update_local_root(path, s.security.allow_symlinks_outside_root)
                .await
        ...
        if !trimmed_temp.is_empty() {
            tokio::fs::create_dir_all(trimmed_temp)
                .await
                .map_err(|e| anyhow::anyhow!("Failed to create temp directory: {}", e))?;
            state
                .set_system_setting("temp_dir", trimmed_temp)
                .await
        ...
        state
            .set_system_setting(
                "allow_symlinks",
                if s.security.allow_symlinks_outside_root {
                    "true"
                } else {
                    "false"
                },
        ...
        state
            .set_system_setting(
                "read_only_default",
                if s.security.read_only_default {
                    "true"
                } else {
                    "false"
                },
        ...
        // Advanced
        state
            .set_system_setting("log_level", &s.advanced.log_level)
            .await
        ...
        let allow_symlinks = payload
            .allow_symlinks
            .unwrap_or(state.config.security.allow_symlinks_outside_root);
        ...
        if let Some(new_root_str) = payload.local_root {
            ...
            if !trimmed.is_empty() {
                ...
                state
                    .update_local_root(path, allow_symlinks)
                    .await
                    .map_err(|e| anyhow::anyhow!("Failed to update local root: {}", e))?;
                updated_keys.push("local_root");
            }
            ...
            state
                .set_system_setting("allow_symlinks", if symlinks { "true" } else { "false" })
                .await
        ...
        if let Some(temp) = payload.temp_dir {
            ...
            if !trimmed.is_empty() {
                tokio::fs::create_dir_all(trimmed)
                    ...
                    .map_err(|e| anyhow::anyhow!("Failed to create temp directory: {}", e))?;
                state
                    .set_system_setting("temp_dir", trimmed)
                    .await
        ...
        if let Some(ro) = payload.read_only_default {
            state
                .set_system_setting("read_only_default", if ro { "true" } else { "false" })
                .await

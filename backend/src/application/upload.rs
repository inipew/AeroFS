...
impl UploadApplicationService {
    ...
    pub async fn create_session(
        ...
        let guard = state.upload_locks.try_acquire(connection_id, &target.path).await?;
        let job = state.transfer_manager.create_inline_upload_job_with_plan(
            Some(user_id.to_string()), file_name.clone(), connection_id.to_string(),
            target.path.clone(), total_bytes, plan.clone(),
        ).await;
        let session = crate::services::UploadSession {
            job_id: job.id.clone(), user_id: user_id.to_string(), connection_id: connection_id.to_string(),
            target, file_name, total_bytes, max_upload_bytes: state.config.limits.max_upload_size,
            target_exists, target_perms, plan,
        };
        // Avoid a second acquire; transfer the guard into the reservation map.
        state.upload_locks.reserve_existing(session.clone(), guard).await;
        Ok(session)
    }
        ...
        S: Stream<Item = Result<Bytes, AppError>> + Send,
    {
        ...
        if session.connection_id != connection_id {
            ...
            return Err(AppError::NotFound(format!("Upload session '{}' not found", job_id)));
        }
        let provider = state.get_provider(&session.connection_id).await.ok_or_else(|| {
            AppError::Vfs(crate::errors::VfsError::ConnectionError(format!(
                "Connection '{}' not found", session.connection_id
            )))
        })?;
        let cancel_token = match state.transfer_manager.cancel_token(&session.job_id) {
            ...
            None => {
                state.upload_locks.release(job_id).await;
                state.transfer_manager.fail_inline_job(job_id, "transfer cancellation token missing".into()).await;
                return Err(AppError::Internal(anyhow::anyhow!("transfer cancellation token missing")));
            }
        };
        let result = crate::transfer::executor::execute_inline_upload_stream(
            ...
            crate::transfer::executor::InlineUploadContext {
                target: session.target.clone(), job_id: session.job_id.clone(), plan: session.plan,
                total_hint: session.total_bytes, max_bytes: session.max_upload_bytes,
                target_exists: session.target_exists, target_perms: session.target_perms,
            },
            cancel_token,
            byte_stream,
        ).await;
        ...
        match result {
            Ok(_) => {
                state.metadata_cache.invalidate(&session.connection_id, &session.target.path).await;
                crate::auth::record_audit_log(&state.db, Some(user_id), "FILE_UPLOAD", Some(&session.connection_id),
                    Some(&session.target.path), "SUCCESS", None,
                    Some(&format!("Uploaded: {} via Transfer {}", session.target.path, session.job_id))).await;
            ...
            Err(error) => {
                if matches!(error, AppError::Cancelled(_)) {
                    ...
                    state.transfer_manager.fail_inline_job(job_id, error.to_string()).await;
                }
                Err(error)
            }
        }
    ...
    pub async fn execute_inline_stream(
            ...
            .cancel_token(&job_id)
        {
            ...
            None => {
                ...
                state.transfer_manager.fail_inline_job(&job_id, error.clone()).await;

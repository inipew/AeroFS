...
#[derive(Debug, Clone)]
pub struct StatFileCommand { pub connection: ConnectionId, pub path: String }

#[derive(Clone)]
pub struct StatFile { authorization: Arc<dyn Authorization>, filesystem: Arc<dyn FileSystemResolver> }

impl StatFile {
    pub fn new(authorization: Arc<dyn Authorization>, filesystem: Arc<dyn FileSystemResolver>) -> Self { Self { authorization, filesystem } }
    pub async fn execute(&self, actor: &Actor, command: StatFileCommand) -> Result<FileMetadata, AppError> {
        ...
        self.filesystem.resolve(&command.connection).await?.stat(&path).await.map_err(Into::into)
    }
}

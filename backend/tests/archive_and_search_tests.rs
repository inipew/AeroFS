...
async fn test_archive_overwrite_modes() {
    let (app, cookie, runtime) = setup_app().await;
    let storage_dir = runtime.temp.path().join("storage");
    ...
    std::fs::write(target_dir.join("collision.txt"), b"Existing Content in Target").unwrap();
    ...
    assert_eq!(std::fs::read(target_dir.join("collision.txt")).unwrap(), b"Existing Content in Target");
    ...
    assert_eq!(std::fs::read(target_dir.join("collision (1).txt")).unwrap(), b"Original Content in Zip");
}

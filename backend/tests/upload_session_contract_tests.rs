...
async fn upload_session_admits_streams_and_completes_the_same_transfer_job() {
    ...
    let state = AppState::new_with_db(
        config,
        init_db(&format!("sqlite://{}?mode=rwc", db_path.display()))
            .await
            .unwrap(),
    )
    .await;
    ...
    let admit = Request::builder()
        ...
        .header(header::CONTENT_TYPE, "application/json")
        .body(Body::from(
            json!({"path":"/session.txt","file_name":"session.txt","total_bytes":5}).to_string(),
        ))
        .unwrap();
    ...
    let jobs: Value =
        serde_json::from_slice(&to_bytes(jobs.into_body(), usize::MAX).await.unwrap()).unwrap();
    let job = jobs
        .as_array()
        .unwrap()
        .iter()
    ...
    assert_eq!(
        std::fs::read(storage.join("session.txt")).unwrap(),
        b"hello"
    );
}

...
fn app_state_exposes_narrow_substate_foundation() {
    ...
    assert!(state.contains("FromRef<AppState> for SearchState"));
    assert!(state.contains("FromRef<AppState> for HealthState"));
...
fn search_service_uses_ports_and_owns_its_limiter() {
    ...
    for forbidden in ["AppState", "check_permission", "ProviderRegistry", "state.db"] {
        assert!(
            ...
            "search service leaked concrete dependency: {forbidden}"
        );
    }
}

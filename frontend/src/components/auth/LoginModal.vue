<template>
  <Transition name="ios-modal" appear>
    <div class="fixed inset-0 z-50 bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4 select-none font-sans">
      <!-- Login Card -->
      <div class="modal-card bg-slate-900 border border-slate-800 rounded-3xl max-w-sm w-full p-8 shadow-2xl relative overflow-hidden ring-1 ring-white/10">
        ...
        <form @submit.prevent="handleLogin" class="space-y-4 relative z-10">
          <div>
            <label class="block text-slate-300 text-xs font-semibold mb-1.5">Username</label>
            <input
              v-model="username"
              type="text"
              placeholder="Username"
              class="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-4 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-xs font-medium"
              required
            />
          </div>

          <div>
            <label class="block text-slate-300 text-xs font-semibold mb-1.5">Password</label>
            <input
              v-model="password"
              type="password"
              placeholder="••••••••"
